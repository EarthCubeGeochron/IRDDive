...
After cleaning the corpus, here are the sites that we pull out from GeoDeepDive. We find 13 papers with 46 coordinate pairs out of 150 documents in the IRDDive test dump. We still have limitations to the current methods.  First, it appears we are finding papers where IRD is simply mentioned, and it is not core data. To circumvent this issue, we need to know where in these papers IRD is being referred to. Perhaps we can target certain parts of the paper, like the Methods, to ensure we are getting coordinates for IRD data. Additionally, some papers mention IRD in the core data for continental cores (see Central Asia location). Perhaps by stripping documents that mention 'continental (place name)' we can clean this further. One last step is to to obtain documents that mention IRD, but as 'IRD-rich layers' by using a regex. Once these last few issues are sorted out, we can begin to pull dates and provenance information from the documents.  
...
```{r}
date <- str_detect(full_nlp$word, "BP")

browse(x = data.frame(gddid = x$`_gddid`[date_range], 
                      words = x$word[date_range]), 
       pubs = publications)

x$word[date&ird_word][4]
