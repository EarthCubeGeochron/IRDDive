...
```{r, regex_apply}
degmin <- str_match_all(full_nlp$word, dms_regex)
decdeg <- str_match_all(full_nlp$word, dd_regex)
```
...
date_range <- str_extract_all(full_nlp$word,
              ...
              bind_rows() %>%
              mutate(gddid = full_nlp$`_gddid`,
                     sentence = full_nlp$sentence)

number <- str_extract_all(full_nlp$word,
                          ",(\\d+(?:[\\.\\s]\\d+){0,1}),.*?,yr.*?,") %>%
              map(function(x) {
...
To circumvent this issue, we need to know where in these papers IRD is being mentioned.

```{r}
locations <- things %>%
  select(gddid, word, sentence) %>%
  left_join(x = total_count, y = ., by=c("gddid", "sentence")) %>%
