...
number <- str_extract_all(nlp$word,
                          ",(\\d+(?:[\\.\\s]\\d+){0,1}),.*?,yr.*?,") %>%
              map(function(x) {
                data.frame(year = paste(x, collapse = ";"),
                           stringsAsFactors = FALSE)
                }) %>%
              bind_rows() %>%
              mutate(gddid = nlp$`_gddid`,
...
total_count <- number %>%
  group_by(gddid) %>%
  summarise(n = n()) %>%
  left_join(x = ., y = number,  by = c('gddid')) %>%
...
## Further Cleaning and Document Metadata



```{r}
locations <- total_count %>%
  select(gddid, words, sentence) %>%
  left_join(x = coordinates_df, y = ., by=c("gddid", "sentence")) %>%
  mutate(year = ifelse(year == "", NA, year),
         range = ifelse(range == "", NA, range),
         interval = round(paper_loc, 2))  %>%
  group_by(interval) %>%
  summarise(age = sum(!is.na(year)),
            age_range = sum(!is.na(range)),
            coordinate = sum(!is.na(words))) %>%
  tidyr::gather(match, count, -interval)

  ggplot(locations, aes(x = interval, y = count, fill = match)) +
    geom_smooth(aes(color = match), fill='gray') +
    geom_bar(stat='identity', position = 'dodge', color = 'gray50') +
    coord_cartesian(xlim=c(0,1), ylim=c(0, 17), expand = FALSE) +
    theme_bw()
...
Targeting certain parts of the paper, like the Methods, might ensure we are only obtaining coordinates for IRD data (see Figure 4).
