...
The response to this API call is a JSON object that indicates the total number of “hits” of the term (n = `r ird_hits <- ird_alone$success$hits` as of `r Sys.Date()`), basic bibliographic citation information for each document containing the term, including link to original PDF distributed by the publisher, and a “snippet” of text around mentions of the term in the full text of the document:
...
To combine multiple terms and require that they are both present in a document.  The following API call returns `r `r ird_hits <- ird_alone$success$hits` as of `r Sys.Date()`, far fewer than the more expansive search.
